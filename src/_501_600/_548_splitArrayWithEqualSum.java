...
public boolean splitArray(int[] nums) {
    ...
    for (int i = 0; i < nums.length-6; i++) {
        ...
        sum += nums[i];
        boolean res = helper(nums, i+2, 2, sum);
        if (res) return res;
    }
    return false;
}

private boolean helper(int[] nums, int start, int sub, int target) {
    if (sub == 0) {
        int sum = 0;
    ...
    int sum = 0; 
    for (int i = start; i < nums.length-sub*2; i++) {
        sum += nums[i];
        if (sum == target) {
            if (sub > 0 && helper(nums, i+2, sub-1, target)){
                return true;
            }
        }
    }
